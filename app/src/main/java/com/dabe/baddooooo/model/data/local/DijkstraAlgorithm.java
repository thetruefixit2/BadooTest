...
public class DijkstraAlgorithm {
    ...
    public void execute(Vertex source) {
        ...
        distance = new HashMap<Vertex, BigDecimal>();
        predecessors = new HashMap<Vertex, Vertex>();
        distance.put(source, new BigDecimal(0d));
    ...
    private void findMinimalDistances(Vertex node) {
        ...
        for (Vertex target : adjacentNodes) {
            BigDecimal temp = getShortestDistance(node).add(getDistance(node, target));
            if (getShortestDistance(target).compareTo(temp) == 1) {
